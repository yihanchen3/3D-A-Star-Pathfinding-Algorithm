...
bool Star::Creatgraph()
{
	...
	ifstream infile; 
	infile.open("D:\inputt.txt", ios::in); //打开文件
	if (!infile.is_open())
	{
	...
	for ( int i = 0 ; i < zDepth ; i++ )
	{
		for ( int j  = 0; j < yDepth ; j++ )
		{
			for ( int k = 0 ; k < xDepth ; k++ )
			{
				...
				pi.yPos = j ;
				pi.zPos = i ;
				bool isWalkable = aray[k][j][i] >= 1 ? true : false;//1为通，0为阻
	...
	startp = nodes[startpointx][startpointy][startpointz];			/// 设置开始节点
	endp   = nodes[endpointx][endpointy][endpointz];			/// 设置结束节点
...
bool Star::Find_path(Node3D * node)
{
	if ( *node != endp  && endp.getWalkable() )
	{
		do 
		{
			...
			int	ex = min(node->point.xPos +1 ,xDepth-1);//点不能出界
			//不能回走（掉头）
			for ( int i = node->point.zPos-1; i <= ez ; i++ )
			{
				for ( int j  = node->point.yPos-1; j <= ey ; j++ )
				{
					...
					{
						if ( !( i == node->point.zPos && j== node->point.yPos && k == node->point.xPos ) && !(j == node->point.yPos && k == node->point.xPos))//不能是点本身,不能垂直向上走
						{
			...
			Node3D * best = NULL;
			if ( open.size() >= 1 )//如果点集里面有点，则取出原来的第一个点，放入新的点
			{
				best = open.begin()->first;
				open.erase( open.begin() );//删除容器中position所指位置的元素。返回值是指向被删元素之后的那个元素(即下一个元素)的迭代器
			}
			node = best;
		} while ( node != NULL  && (*node != endp)) ;
	}

	if ( node != NULL  && endp.getWalkable() && (*node == endp))//路线连接到终点了
	{
...
void Star::searchchNode(Node3D * adjNode  , Node3D * current)
{
	...
	bool xZDiagonal = ( adjNode->point.xPos != current->point.xPos && adjNode->point.zPos != current->point.zPos && adjNode->point.yPos == current->point.yPos);//bool返回true 或 false
	...
	if(  adjNode->getWalkable() &&  !findItem(open,adjNode) && !findItem(close,adjNode) && !corner)
	{
		...
		{
			adjNode->g = current->g + (xZDiagonal || xYDiagonal || yZDiagonal) ? 14 : 10;//条件运算符 如果为真则等于14
		}
	...
	else if  ( adjNode->getWalkable() && !findItem(close,adjNode) && !corner ) 
	{
		int g;
			...
			{
				g = current->g + (xZDiagonal || xYDiagonal || yZDiagonal) ? 14 : 10;//条件运算符 如果为真则等于14
			}
		if ( g < adjNode->g )
		{
			adjNode->g = g;
